...
abstract public class Lisp extends ABCLStatic {
	...
	private static final void pushJavaStackFrames() {
		...
		// frames[1] abcl.LispThread.getJavaStackTrace
		// frames[2] abcl.Lisp.pushJavaStackFrames

		if (frames.length > 5 && frames[3].getClassName().equals("abcl.Lisp") && frames[3].getMethodName().equals("error") && frames[4].getClassName().startsWith("abcl.Lisp") && frames[4].getMethodName().equals("eval")) {
		...
		for (int i = 0; i <= last; i++) {
			if (frames[i].getClassName().startsWith("abcl.Primitive"))
				last = i;
		}
...
//	}
	...
	protected static final ConcurrentHashMap<String, LispObject> objectTable = new ConcurrentHashMap<String, LispObject>();
	...
	// Used in abcl/j/JLisp.java.
	public static final void resetIO() {
		//resetIO(Stream.createStream(Symbol.SYSTEM_STREAM, System.in, Symbol.CHARACTER, true), Stream.createStream(Symbol.SYSTEM_STREAM, System.out, Symbol.CHARACTER, true));
	}
	...
	public static void initSystemIO() {
		Stream stdin = Stream.createStream(Symbol.SYSTEM_STREAM, System.in, Symbol.CHARACTER, true);
		Stream stdout = Stream.createStream(Symbol.SYSTEM_STREAM, System.out, Symbol.CHARACTER, true);
		Stream stderr = Stream.createStream(Symbol.SYSTEM_STREAM, System.err, Symbol.CHARACTER, true);
...
//	public static Primitive pro = new print_readable_object();
	...
	public static boolean isTooSoon() {
		return Lisp.cold || !Lisp.initialized || !abcl.Interpreter.topLevelInitialized;
	}
