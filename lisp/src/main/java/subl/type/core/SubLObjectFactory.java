	...
	public static SubLNumber makeBasedIntegralNumber(String str) {
		...
		if (Character.isDigit(secondChar)) {
			if (Character.isDigit(thirdChar))
				return makeIntegralNumber(str.substring(4), Integer.parseInt(String.valueOf(secondChar) + thirdChar));
			return makeIntegralNumber(str.substring(3), Integer.parseInt(String.valueOf(secondChar)));
		} else {
			switch (secondChar) {
				case 'b': case 'B':
...
//    }
	...
	public static SubLList makeList(Iterable<? extends SubLObject> i) {
		return makeList(i.iterator());
	}

	public static SubLList makeList(Iterator<? extends SubLObject> i) {
		Threads.possiblyHandleInterrupts(true);
		SubLList result = SubLNil.NIL;
		while (i.hasNext()) {
			result = new Cons(i.next(), result);
		}
		return result;
	}
	...
	public static SubLCons makeListS(SubLObject arg, SubLObject arg1, SubLListListIterator finalCdr) {
		...
		ArrayList<SubLObject> listInternals = new ArrayList<>(objectCount + 1);
	...
	public static SubLObject makeCycConstant(final String constant_name) {
		...
		throw new UnsupportedOperationException("TODO: MakeCycConstant " + constant_name);
	}
	...
	public static final SubLObjectFactory me;
	...
	private static final Map<String, SubLInteger> integerParsingCache;
//	private static Map charCache;
	private static int MAX_CHAR_PARSING_CACHE_SIZE = 1000;
	private static final Map<String, SubLCharacter> charParsingCache;
	private static long uniquifier;
	private static final int MAX_POSITIVE_INTEGER_LENGTH;
	private static final int MAX_NEGATIVE_INTEGER_LENGTH;
	private static final int MAX_POSITIVE_LONG_LENGTH;
	private static final int MAX_NEGATIVE_LONG_LENGTH;
...
//		charCache = new HashMap();
		...
		MAX_POSITIVE_INTEGER_LENGTH = "2147483647".length();
		MAX_NEGATIVE_INTEGER_LENGTH = "-2147483648".length();
		MAX_POSITIVE_LONG_LENGTH = "9223372036854775807".length();
		MAX_NEGATIVE_LONG_LENGTH = "-9223372036854775808".length();
	}
}
